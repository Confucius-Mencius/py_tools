...
'''
        ...
        if msg.endswith('Req'):
            ...
            nfy_instance = name_style_util.camel_to_underline(msg).lower().replace('req', 'rsp')
            content = ['    def __make_%s(self, rsp_msg_body):' % nfy_instance.replace('req', 'rsp'), os.linesep,
                       '        %s = cs_msg_pb2.%s()' % (nfy_instance, msg.replace('Req', 'Rsp')), os.linesep,
                       '        %s.ParseFromString(rsp_msg_body)' % nfy_instance, os.linesep * 2,
            ...
            content = ['    def %s(self):' % req_instance.replace('_req', ''), os.linesep,
                       ...
                       '        msg_head = MsgHead()', os.linesep,
                       '        msg_head.msg_id = cs_msg_id_pb2.MSG_ID_%s' % name_style_util.camel_to_underline(
                           msg).upper(), os.linesep * 2,
            ...
            content = ['        if ret != 0:', os.linesep,
                       ...
                       '            return -1', os.linesep * 2,
                       '        if rsp_msg_head.msg_id != cs_msg_id_pb2.MSG_ID_%s:' % name_style_util.camel_to_underline(
                           msg).upper().replace('REQ', 'RSP'), os.linesep,
                       '            LOG_ERROR(\'error rsp msg id: %d\' % rsp_msg_head.msg_id)', os.linesep,
                       '            return -1', os.linesep * 2,
                       '        %s = self.__make_%s(rsp_msg_body)' % (nfy_instance, nfy_instance), os.linesep * 2,
                       '        err_code = %s.err_ctx.err_code' % nfy_instance, os.linesep,
                       '        if err_code != 0:', os.linesep,
                       '            LOG_ERROR(\'%s failed err code: %%d\' %% err_code)' % nfy_instance, os.linesep,
            ...
            content = ['        if ret != 0:', os.linesep,
                       '            LOG_ERROR(\'ret: %d\' % ret)', os.linesep,
                       '            return -1', os.linesep * 2,
                       '        if nfy_msg_head.msg_id != cs_msg_id_pb2.MSG_ID_%s:' % name_style_util.camel_to_underline(
                           msg).upper(), os.linesep,
                       '            LOG_ERROR(\'error nfy msg id: %d\' % nfy_msg_head.msg_id)', os.linesep,
                       '            return -1', os.linesep * 2,
                       '        %s = self.__make_%s(nfy_msg_body)' % (nfy_instance, nfy_instance), os.linesep * 2,
                       '        return 0', os.linesep]
            fp.write(''.join(content).encode('utf-8'))
