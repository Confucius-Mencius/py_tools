...
def output_content(fp, xls_head_, xls_row_grid, xls_col_grid, indent):
    ...
    n = 0
    nrows = len(xls_row_grid.row_list)

    for xls_row in xls_row_grid.row_list:
        n += 1

        if n != nrows:
            content = '%s"%d": "%s",%s' % (indent,
...
def generate_err_code_json_file(xls_head_, xls_row_grid, xls_col_grid, output_dir):
    file_path = os.path.join(output_dir, 'err_code.json')
...
def test_001():
    ...
    assert 0 == generate_err_code_json_file(xls_head_, xls_loader.row_grid, xls_loader.col_grid, './output/demo_proj')
