...
def output_content(fp, xls_head_, xls_row_grid, xls_col_grid, indent):
    for xls_row in xls_row_grid.row_list:
        content = '_dict[%d] = "%s";%s' % (xls_row.get_cell(xls_head_.conf['value']['col']).content,
                                           xls_row.get_cell(xls_head_.conf['desc_en']['col']).content,
                                           os.linesep)
        fp.write(content.encode('utf-8'))


def generate_err_code_csharp_file(xls_head_, xls_row_grid, xls_col_grid, output_dir):
    file_path = os.path.join(output_dir, 'err_code.cs')
...
def test_001():
    ...
    assert 0 == generate_err_code_csharp_file(xls_head_, xls_loader.row_grid, xls_loader.col_grid, './output/demo_proj')
