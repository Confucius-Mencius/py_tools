...
def output_content(fp, xls_head_, xls_row_grid, xls_col_grid, indent):
    content = ['#ifndef COMMON_UTIL_INC_ERR_CODE_H_', os.linesep,
               '#define COMMON_UTIL_INC_ERR_CODE_H_', os.linesep * 2]
    fp.write(''.join(content).encode('utf-8'))

    for xls_row in xls_row_grid.row_list:
        content = '#define %-40s %-3d    // %s%s' % (xls_row.get_cell(xls_head_.conf['name']['col']).content,
                                                     xls_row.get_cell(xls_head_.conf['value']['col']).content,
                                                     xls_row.get_cell(xls_head_.conf['desc_en']['col']).content,
                                                     os.linesep)
        fp.write(content.encode('utf-8'))

    content = [os.linesep,
               '#endif // COMMON_UTIL_INC_ERR_CODE_H_', os.linesep]
    fp.write(''.join(content).encode('utf-8'))


def generate_err_code_h_file(xls_head_, xls_row_grid, xls_col_grid, output_dir):
    file_path = os.path.join(output_dir, 'err_code.h')
...
def test_001():
    ...
    assert 0 == generate_err_code_h_file(xls_head_, xls_loader.row_grid, xls_loader.col_grid, './output/demo_proj')
