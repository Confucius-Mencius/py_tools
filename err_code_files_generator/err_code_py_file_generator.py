...
'''
    ...
    for xls_row in xls_row_grid.row_list:
        content = '%s = %d  # %s%s' % (xls_row.get_cell(xls_head_.conf['name']['col']).content,
...
def generate_err_code_py_file(xls_head_, xls_row_grid, xls_col_grid, output_dir):
    file_path = os.path.join(output_dir, 'err_code.py')
...
def test_001():
    ...
    assert 0 == generate_err_code_py_file(xls_head_, xls_loader.row_grid, xls_loader.col_grid, './output/demo_proj')
